...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, changes = [], startupId, startup, numfields;
                ...
                if (changes.length > 0) {
                    ...
                    if (tmp.length > 0) {
                        ...
                        if (rslt.status) {
                            ...
                            if (Helper.IsNullValue(mapItem.navpropname)) startupId = rslt.id;
                            ...
                            rslt = await Support.AddOrUpdateStartup(data, dropDownOptions);
        ...
        if (changes.length > 0 && changes.indexOf('Logo') > -1) {
            data = startup.find((x) => x.key === 'Logo');
            rslt = await Support.AddOrUpdateDocument(data);
            if (rslt.status) {
                let newImageId = parseInt(rslt.id);
                data = [
                    { key: "SId", value: parseInt(startupId) },
                    { key: "StartupLogo", value: newImageId }
                ];
                rslt = await Support.AddOrUpdateStartup(data);
                if (!rslt.status) return;

                let newValues = startup.find((x) => x.key === 'Logo').value;
                newValues = { ...newValues, DocId: newImageId };
                row['startup'].find((x) => x.key === 'Logo').value = newValues;
                                // Update Back for next tracking purpose
                UpdateBackUp('startup');
        ...
        global.AlertPopup("success", "Startup is updated successfully!");
        setShowUpdate(false);
        NavigateTo(-1);
    }
